...
def limit_chart(df, values, x_labels, USL, LSL, target, title='Limit Chart', y_label='Value', 
                     x_label='', figsize=(15,3), round_value=4, dpi=300, show_limit_values=True):
    
    """
    Generates a limit chart that sequentially plots process data in the context of the provided specification limits and target.
    ...
    try:
        ...
        if not np.issubdtype(data.dtype, np.number):
            raise TypeError(f"Column '{values}' must contain numeric values.")
        if not np.issubdtype(labels.dtype, np.number) and not labels.dtype == object:
            raise TypeError(f"Column '{x_labels}' must be numeric or string.")

        # Ensure USL, LSL, and target are valid numbers
        if not all(isinstance(val, (int, float)) for val in [USL, LSL, target]):
            raise TypeError("USL, LSL, and target must be numbers.")

        # Validate target within limits
        if not (LSL <= target <= USL):
            raise ValueError(f"Target value {target} must be between LSL ({LSL}) and USL ({USL}).")
        ...
        annotations = [
            (limit_labels[0], xlimit, USL, ax),  # UPL annotation on ax[0]
            (limit_labels[1], xlimit, LSL, ax),  # LPL annotation on ax[0]
            (limit_labels[2], xlimit, target, ax)
        ]
